...
import { importDispatch } from '../../import-test/dispatch';

const config = readConfigSync();

const DEFAULT_OUTDIR = 'import-tests';
const LANGUAGES = ['typescript', 'python', 'java'];

class Command implements yargs.CommandModule {
  public readonly command = 'import-test [SPEC]';
  public readonly describe = 'Imports API object tests for your app by generating tests from sample YAML.';
  public readonly aliases = ['gen-test', 'import-test', 'generate-test'];

  public readonly builder = (args: yargs.Argv) => args
    .positional('SPEC', { default: config.testImports, desc: 'import-test spec with the syntax [NAME:=]SPEC where NAME is an optional module name and supported SPEC is the input directory of sample yaml to recursively import from.', array: true })
    .example('cdk8s import-test test_cr.yaml', 'Imports Kubernetes API objects to imports/k8s.ts')
    .example('cdk8s import-test test/fixtures', 'Imports Kubernetes API objects to imports/k8s.ts')
    .example('cdk8s import-test --ignore /**/examples/* test/fixtures', 'Imports Kubernetes API objects to imports/k8s.ts')
