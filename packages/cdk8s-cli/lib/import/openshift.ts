...
const DEFAULT_API_VERSION = '3.11.0';

export interface ImportOpenShiftApiOptions {
  /**
...
export class ImportOpenShiftApi extends ImportBase {

  public static async match(importSpec: ImportSpec, argv: any): Promise<ImportOpenShiftApiOptions | undefined> {
    const { source } = importSpec;
    if (source !== 'openshift' && !source.startsWith('openshift@')) {
      return undefined;
    }
  ...
  constructor(private readonly options: ImportOpenShiftApiOptions) {
    super()
  }
...
async function downloadSchema(apiVersion: string) {
  const url = `https://raw.githubusercontent.com/garethr/openshift-json-schema/master/v${apiVersion}/_definitions.json`
